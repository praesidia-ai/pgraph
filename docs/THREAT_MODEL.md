...
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Core               | Root/config/store composition: `packages/graph-core/src/index.ts:60`                                                                                                             |
| Discovery/compiler | Hash eligible source; parse JSON compiler settings and source without executing them: `packages/graph-indexer/src/discovery.ts:32`, `packages/graph-typescript/src/index.ts:225` |
...
| Editor worker      | Trusted Node process for responsiveness/cancellation; inherited host privileges: `apps/vscode-extension/src/client.ts:54`                                                        |
| Copilot            | Explicit model selection and source-derived evidence transmission: `apps/vscode-extension/src/extension.ts:347`, `apps/vscode-extension/src/extension.ts:396`                    |
...
| ------------------------- | ----------------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| CLI/editor indexing       | Persistent graph              | Operator/host root → realpath → fixed child path                            | R/.pgraph/graph.db, graph.db-wal, graph.db-shm                          | Local engine / OS-authorized users                     | Symlink checks; new directory mode 0700; transactions and schema checks                           | `packages/graph-core/src/index.ts:60`, `packages/graph-store-sqlite/src/index.ts:23`; existing directory modes are not reset       |
...
| Initialization            | Marker                        | Fixed child path, written only if absent                                    | R/.pgraph/config.json                                                   | Local filesystem                                       | Mode 0600, root checks                                                                            | `packages/graph-core/src/index.ts:60`; actual settings are R/.pgraph.json                                                          |
...
`packages/graph-tools/src/index.ts:191`, `packages/graph-typescript/src/index.ts:106`, `apps/vscode-extension/src/extension.ts:347`.
...
| Medium   | Executable/environment resolution runs unintended trusted-tool binary | Compromised machine PATH/config or explicit provider choice                          | Host process authority                             | No shell/repo scripts; machine nodePath; fixed git arguments; default Git disabled                             | Trust installed tools/host configuration; subprocess separation is not privilege isolation                  | `apps/vscode-extension/src/client.ts:54`, `packages/graph-git/src/index.ts:18`, `packages/graph-core/src/index.ts:173`                   |
| Low      | Cleanup/logging races disrupt local graph consumers or grow disk      | Operator cleans while clients remain open, or enables prolonged diagnostics          | Recoverable local index/disk availability          | Cleanup only known graph filenames; transactions; bounded SQLite run history                                   | Stop clients before clean; use rebuild normally; add log rotation                                           | `packages/graph-core/src/index.ts:91`, `packages/graph-indexer/src/index.ts:230`                                                         |
